...
We also provide a labelling interface which loads the the generated jpeg files and uses keyboard inputs to perform labelling.

Instructions on use:
Copy subset which you want to label into a separate folder <br />
Set path to this folder in the path.py file
Run GUI.py

Labelling:
Keyboard inputs 1 through to 5 for noise classification
Keyboard inputs "qwert" for rhythm classification, with mapping :

Navigation buttons (operated by the left and right keyboard buttons) allow for navigation through the labels.

It is also possible to click the radio buttons using the mouse. 

Once the labelelling is done, the submit button commits these to a csv file which stores the labels.
